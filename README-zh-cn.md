...
<h2 style="margin-top: 0" align="center"><strong>中文</strong> | <a href="./README.md">English</a></h2>

<p align="center">Dynamite音乐游戏谱面图片生成工具 (C++ 实现)</p>
...
│     └─ dynachart_renderer_legacy.cpp  # 旧式渲染器(v1.1.0及以前)实现
├─ src/
...
选项:
  -s <float>, --scale <float>        缩放比例 (默认：0.25)
  -S, --speed <float>       显示速度 (决定小节高度，默认：0.5)
  -b, --bar-span <int>      高亮小节线间隔 (默认：2)
...
DynaChartView_Setup_xxx.exe - Windows 安装包（包含可执行文件和必要的运行时库，安装后会在系统中注册环境变量）
...
* 🐛 **拒绝非法输入**。-s, -S, -l ,-b 参数现在会检查输入值是否合法（例如缩放比例必须大于0，每页小节数必须为正整数等）。如果输入不合法，程序会输出错误信息并退出，而不是继续执行导致不可预期的行为。
* 🎨 **修正图层顺序** - 现在的Note图层顺序为：HOLD底层，NOTE中层，CHAIN顶层。这样不会出现CHAIN被其它Note覆盖的情况了。
* 🔄 **可选v1.1.0渲染** - 通过新增--legacy-render选项，用户可以选择使用旧式渲染方式（v1.1.0版本的渲染逻辑），以兼容某些特殊需求或喜欢旧式视觉效果的用户。
